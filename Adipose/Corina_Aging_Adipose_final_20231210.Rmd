---
title: "scRNA - Seurat - uPAR Aging scRNA"
author: "Ray"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: html_document
params:
    workdir: "./"
    prefix: "Corina_Aging_Adipose_final"
    dat.path: "/Users/hoy/Copy/MSKCC/Corina/Aging/GitHub/Adipose"
    sample.file: "/Users/hoy/Copy/MSKCC/Corina/Aging/GitHub/Adipose/SampleID.csv"
    geneset.file: "/Users/hoy/Copy/MSKCC/Corina/Aging/GitHub/Adipose/AdiposeTissue_Signatures.xlsx"
    geneset.sheet: "Emont_top30"
    # geneset.sheet: "celltype.l2.mouse"
    # geneset.sheet: "celltype.l1.mouse"
---
...
mat <- Seurat::Read10X_h5("/Users/hoy/Copy/MSKCC/Corina/Aging/GitHub/Adipose/Liver_Adipose_filtered_feature_bc_matrix.h5")
...
pbmc <- subset(pbmc, subset = Type %in% c("AT1", "AT3"))
...
# save 
qs::qsave(pbmc, paste0(prefix, ".rawdata.SCTransform.BBKNN.qs"))
...
pbmc <- qs::qread(paste0(prefix, ".rawdata.SCTransform.BBKNN.qs"))
DefaultAssay(pbmc) <- "RNA"
...
qs::qsave(pbmc, paste0(prefix, ".rawdata.SCTransform.BBKNN.singlr.qs"))
```



### Calculate ModuleScore 

```{r module_score, message=FALSE}
pbmc <- qs::qread(paste0(prefix, ".rawdata.SCTransform.BBKNN.singlr.qs"))
pdac_features <- readxl::read_excel(params$geneset.file, sheet = params$geneset.sheet) %>%
  dplyr::select(-all_pericyte)
...
pbmc <- AddModuleScore(object = pbmc,
                       ...
                       assay = "SCT",
                       name = 'Adipose') %>% 
  replace_ModuleScore_feature_name_v2(., pdac_features, name = "Adipose")

### another version of random shuffle cell order
my.genes <- c("Plaur", "Steap4", "Myocd") # 
p <- FeaturePlot(pbmc, 
                 features = c(my.genes, names(pdac_features)),
                 order = FALSE,
                 pt.size = 0.4,
                 min.cutoff = "q5", 
                 max.cutoff = "q95", 
...
pdf(paste(prefix, params$geneset.sheet, "ModuleScore_FeaturPlot.pdf", sep="."), width=26, height=24)
  print(p)
dev.off()

### avg.ModuleScore
ModuleScore <- pbmc@meta.data[, names(pdac_features)]
...
cell.type <- tibble(ident = pbmc@meta.data$seurat_clusters) %>%
  mutate(Group = case_when(ident == 0 ~ "ASPC",
                           ident == 1 ~ "Macrophage",
                           ident == 2 ~ "Bcell",
                           ident == 3 ~ "Monocyte",
                           ident == 4 ~ "Bcell",
                           ...
                           ident == 18 ~ "Tcell",
                           ident == 19 ~ "Tcell",
                           ident == 20 ~ "Endothelial",
                           ident == 21 ~ "NK",
                           ident == 22 ~ "Pericyte",
                           ident == 23 ~ "DC2",
                           ident == 24 ~ "DC2",
                           ident == 25 ~ "Bcell",
                           ident == 26 ~ "Mesothelial",
                           ident == 27 ~ "ASPC",
                           ident == 28 ~ "ASPC",
                           ident == 29 ~ "Mast",
                           ident == 30 ~ "DC2",
...
p3 <- DimPlot(pbmc, group.by = c("Cell_type", "SampleID", "uPAR", "Phase"), 
...
paper.features <- c("Pecam1", "Cdh5", 
                    ...
                    "Il7r", # CD4_Tcell
                    "Cd3d", "Cd3e", # Tcell
                    "Nkg7", "Ccl5", "Klrd1", "Gzma",  # NK
                    "Lst1", "Il1b", "Fgr", "Ccr1", # Monocyte
                    "Cd45", "Adgre1", "C1qa", "C1qb", # Macrophage
                    "Tnip3", "Ear2", "Slc37a2",  # DC2
                    "Flt3", "Clec9a", "Pmaip1", # DC1
                    "Cpa3", "Tpsb2", "Mrgprb1", # Mast
                    "Rgs5", "Myo1b", "Postn",  # Pericyte
                    "Msln", "Pkhd1l1", "Slpi", # Mesothelial
                    "Cd19", "Ms4a1", "Cd79b", # Bcell
                    "Jchain", "Derl3", "Fkbp11") 
paper.celltype <- c("Endothelial", "ASPC", 
                    "Tcell", "NK", 
                    "Monocyte", "Macrophage", 
                    "DC2", "DC1", "Mast", "Pericyte", "Mesothelial",
                    "Bcell", "Plasma")

pbmc$Cell_type <- factor(pbmc$Cell_type, levels = rev(paper.celltype))

p <- DotPlot(pbmc, features = paper.features, group.by = "Cell_type",
             cols = c("Reds"), dot.scale = 8.2) + 
  RotatedAxis() + 
  xlab("") +
  ylab("") +
...
pdf(paste0(prefix, ".NewCelltype.Paper_dotplot.pdf"), width=14, height=7)
  print(p)
dev.off()
```
...
p <- res %>% 
  ...
  scale_y_continuous(limits = c(0, 0.95), expand = expansion(mult = c(0, -.1))) +
...
```{r module_score, message=FALSE}
pbmc <- qs::qread(paste0(prefix, ".pbmc.ReIdent.qs"))
geneset.file <- "/Users/hoy/Copy/MSKCC/Corina/Aging/GitHub/Adipose/Senescence_and_SASP_genesets_2022_mouse.xlsx"
...
pdf(paste(prefix, params$geneset.sheet, "ReIdent.ModuleScore_FeaturPlot.pdf", sep="."), width=24, height=16)
  print(p)
dev.off()
```
...
- use 0.1 as cutoff for Senescence score (Senescence_High, Senescence_Low)
...
pbmc <- AddModuleScore(object = pbmc,
                       ...
                       ctrl = 100,
                       name = 'sene') %>%
  replace_ModuleScore_feature_name_v2(., sene_features, name = "sene")
