---
title: "scRNA - Seurat - uPAR Aging scRNA - PDAC"
author: "Ray"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: html_document
params:
    workdir: "./"
    prefix: "Corina_Aging_PDAC_final"
    dat.path: "/Users/hoy/Copy/MSKCC/Corina/Aging/GitHub/Pancreas"
    sample.file: "/Users/hoy/Copy/MSKCC/Corina/Aging/GitHub/Pancreas/SampleID.csv"
    geneset.file: "/Users/hoy/Copy/MSKCC/Corina/Aging/GitHub/Pancreas/PDAC_signatures.xlsx"
    geneset.sheet: "PDAC_Mouse"
---
...
mat <- Seurat::Read10X_h5("/Users/hoy/Copy/MSKCC/Corina/Aging/GitHub/Pancreas/Pancreas_filtered_feature_bc_matrix.h5")
...
meta <- read_csv(params$sample.file) %>%
  separate(SampleID, into = c("tmp1", "tmp2", "tmp3", "tmp4", "tmp5"), sep="_") %>%
  dplyr::rename(Sex = tmp3) %>%
  unite(uPAR, tmp4, tmp5) %>%
  mutate(Group = paste(Sex, uPAR, sep="_")) %>%
  mutate(Type = as.factor(Group) %>% as.numeric() %>% paste0("PDAC", .)) %>%
  mutate(SampleID = paste("Amor", Type, Sex, uPAR, sep="_"), .after = Barcode) %>%
...
pdf(paste0(prefix, ".ModuleScore_FeaturPlot.rawdata.SCTransform.BBKNN.pdf"), width=28, height=18)
  print(p)
dev.off()
```
...
p1 <- DimPlot(pbmc, reduction = "umap", 
              group.by = c("SampleID", 
                           ...
                           "singlr_Mouse_main", 
                           "singlr_BaronPanc"),
              label = TRUE, label.size = 3.5, pt.size = 0.7, 
              ncol = 3) &
  theme(axis.title = element_text(size = 14),
        plot.title = element_text(size = 24)) &
  guides(color=guide_legend(ncol =1, override.aes = list(size=3)))

pdf(paste0(prefix, ".singlR.annotation.pdf"), width=20, height=11)
  print(p1)
dev.off()
...
- calculate [avg modulescore](https://github.com/satijalab/seurat/issues/1695)
- Keep only avg.cluster.modulescore of ??
...
pdf(paste(prefix, params$geneset.sheet, "ModuleScore_FeaturPlot.pdf", sep="."), width=28, height=24)
  print(p)
dev.off()
...
cell.type <- tibble(ident = pbmc@meta.data$seurat_clusters) %>%
  mutate(Group = case_when(ident == 0 ~ "Bcell",
                           ident == 1 ~ "Bcell",
                           ident == 2 ~ "Bcell",
                           ident == 3 ~ "Macrophage",
                           ident == 4 ~ "Bcell",
